const RouteOptimize = (function() {
    ...
    function analyzeBoard(state) {
        ...
        for (const d of detourResults) {
            if (d.isDetour) {
                const originalPoints = (tracks.find(t => t.id === d.trackId) || {}).points || [];
                suggestions.push({
                    type: 'detour',
                    trackId: d.trackId,
                    net: d.net,
    ...
    function applyColinearOptimizationToState(trackIds) {
        const state = PCBState.getState();
        const result = applyColinearOptimization(state, trackIds);
        if (result.optimizedCount > 0) {
            PCBState.setState(result.newState);
        }
        return result;
    }
    ...
    return {
        ...
        applyColinearOptimizationToState,
        analyzeCurrentBoard: () => analyzeBoard(PCBState.getState())
    };
})();
